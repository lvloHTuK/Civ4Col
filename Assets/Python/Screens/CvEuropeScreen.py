...
class CvEuropeScreen:
	...
	def sailToNewWorld(self, iUnit):
		...
		for i in range(0, 6) : 
			if self.EuropePlotListSectorsEast[i]:		#east sector buttons  (frigids,temperates,tropics)			
		...
		for i in range(0, 6) : 
			if self.EuropePlotListSectorsWest[i]:		#west sector buttons (frigids,temperates,tropics)
	...
	def getPlotLists (self, unit) :
		...
		#the sector_All variables are used to hold all discovered plots in a sector. They will be used if a border-to-europe tile is not found in the sector
