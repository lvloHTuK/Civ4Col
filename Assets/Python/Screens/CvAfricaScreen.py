...
## by koma13 
## converted to Africa Screen by ray
...
class CvAfricaScreen:

	def __init__(self):
		self.WIDGET_ID = "AfricaScreenWidget"
		self.nWidgetCount = 0
	...
	def interfaceScreen(self):
		...
		## Africa Screen Configuration START
		...
		if not self.iThisWinter:
			if self.iVideo > 0 and iCurrentTurn % self.iVideo == 0 and not iCurrentTurn == 0 and not CyUserProfile().getGraphicOption(gc.getInfoTypeForString("GRAPHICOPTION_NO_MOVIES")):
				...
				screen.addDDSGFC("AfricaScreenBackground", "Art/Interface/Screens/Africa/FullBackground.dds", 0, 0, self.XResolution, self.YResolution, WidgetTypes.WIDGET_GENERAL, -1, -1 )
	...
	def drawContents(self):
		...
		while (unit):
			if (not unit.isCargo() and not unit.isDelayedDeath()):
				if (unit.getUnitTravelState() == UnitTravelStates.UNIT_TRAVEL_STATE_IN_AFRICA):
					if not unit.getID() in self.AfricaUnitsList:
						self.AfricaUnitsList.append(unit.getID())
				elif (unit.getUnitTravelState() == UnitTravelStates.UNIT_TRAVEL_STATE_FROM_AFRICA):
					if not unit.getID() in self.OutboundUnitsList:
						self.OutboundUnitsList.append(unit.getID())
				elif (unit.getUnitTravelState() == UnitTravelStates.UNIT_TRAVEL_STATE_TO_AFRICA):
		...
		for iUnit in self.AfricaUnitsList:
			...
			for i in range(unit.cargoSpace()):
				screen.addDDSGFCAt(self.getNextWidgetName(), "LoadingList", ArtFileMgr.getInterfaceArtInfo("INTERFACE_EUROPE_BOX_CARGO").getPath(), iX_Space , self.INPORT_SHIP_H * 7 / 12, self.CARGO_ICON_SIZE, self.CARGO_ICON_SIZE, WidgetTypes.WIDGET_SHIP_CARGO_AFRICA, unit.getID(), -1, False)
			...
			for i in range(plot.getNumUnits()):
				...
				if (not transportUnit.isNone() and transportUnit.getID() == unit.getID() and transportUnit.getOwner() == unit.getOwner()):
					...
					screen.addDragableButtonAt("LoadingList", self.getNextWidgetName(), loopUnit.getButton(), "", iX_Cargo , self.INPORT_SHIP_H * 7 / 12, self.CARGO_ICON_SIZE, self.CARGO_ICON_SIZE, WidgetTypes.WIDGET_SHIP_CARGO_AFRICA, loopUnit.getID(), transportUnit.getID(), ButtonStyles.BUTTON_STYLE_LABEL)
		...
		iX = 0
		iNumDocks = player.getNumAfricaUnits()
		if (iNumDocks < 5):
			iNumDocks = 5
	
		for i in range(player.getNumAfricaUnits()):
			loopUnit = player.getAfricaUnit(i)
			iY, iW = 0, self.DOCK_UNIT_W
			UnitInfo = gc.getUnitInfo(loopUnit.getUnitType())
			iProfession = loopUnit.getProfession()
			iUnitArtStyle = gc.getCivilizationInfo(player.getCivilizationType()).getUnitArtStyleType()
			screen.addDragableButtonAt("DockList", self.getNextWidgetName(), UnitInfo.getUnitArtStylesArtInfo(0, iProfession, iUnitArtStyle).getFullLengthIcon(), "", iX, iY, iW, self.DOCK_UNIT_H, WidgetTypes.WIDGET_DOCK_AFRICA, loopUnit.getID(), -1, ButtonStyles.BUTTON_STYLE_LABEL )
			iX += self.DOCK_UNIT_W * 6 / iNumDocks
		
		screen.addDDSGFC("DockOverlay", "Art/Interface/Screens/Africa/DockOverlay.dds", 0, 0, self.XResolution, self.YResolution, WidgetTypes.WIDGET_GENERAL, -1, -1)
		screen.setHitTest("DockOverlay", HitTestTypes.HITTEST_NOHIT)
		
		# Units to Recruit - Not available in Africa
		...
		for i in range(player.getNumTradeMessages()):
			...
			if player.getTradeMessageType(i) == TradeMessageTypes.TRADE_MESSAGE_EUROPE_YIELD_SOLD:
				iPrice *= self.playerEurope.getYieldAfricaBuyPrice(iYield)
				...
				iPrice *= self.playerEurope.getYieldAfricaSellPrice(iYield)
				szBuffer = u"%d%c" % (player.getTradeMessageAmount(i), gc.getYieldInfo(iYield).getChar())
		...
		if (inputClass.getNotifyCode() == NotifyCode.NOTIFY_CLICKED):
			...
			if (inputClass.getButtonType() == WidgetTypes.WIDGET_GENERAL):
				...
				#R&R, vetiarvind, navigation sectors - END				
					...
					if (inputClass.getFunctionName() == "DialogTable") :
						if (not transport.isNone()) and transport.getUnitTravelState() != UnitTravelStates.UNIT_TRAVEL_STATE_FROM_AFRICA:
					...
					(unit, iter) = player.firstUnit()
					while (unit):
						if (unit.getUnitTravelState() == UnitTravelStates.UNIT_TRAVEL_STATE_IN_AFRICA and unit.isCargo() and unit.isGoods()):
							if (unit.getTransportUnit().getID() == transport.getID()):
								CyMessageControl().sendPlayerAction(player.getID(), PlayerActionTypes.PLAYER_ACTION_SELL_YIELD_UNIT_AFRICA, 0, unit.getYieldStored(), unit.getID())
						(unit, iter) = player.nextUnit(iter)
					...
					for i in range(player.getNumAfricaUnits()):
						loopUnit = player.getAfricaUnit(i)
						if (not transport.isNone() and transport.getUnitTravelState() == UnitTravelStates.UNIT_TRAVEL_STATE_IN_AFRICA and not transport.isFull()):
							CyMessageControl().sendPlayerAction(player.getID(), PlayerActionTypes.PLAYER_ACTION_LOAD_UNIT_FROM_AFRICA, loopUnit.getID(), inputClass.getData2(), -1)
					screen.setSoundId(CyAudioGame().Play2DSound("AS2D_REVOLTEND"))

				elif (inputClass.getData1() == self.RECALL) :
					transport = player.getUnit(inputClass.getData2())
					if (not transport.isNone()) and transport.getUnitTravelState() != UnitTravelStates.UNIT_TRAVEL_STATE_IN_AFRICA:
						CyMessageControl().sendApplyEvent(CvUtil.EventDoEuropeScreen, EventContextTypes.EVENTCONTEXT_ALL, (RECALL_TO_AFRICA, transport.getID(), -1, -1, -1, -1, -1, -1, -1))
					...
					if (not transport.isNone() and not iSellPrice == -1):
						CyMessageControl().sendApplyEvent(CvUtil.EventDoEuropeScreen, EventContextTypes.EVENTCONTEXT_ALL, (SELL_SHIP_IN_AFRICA, transport.getID(), -1, -1, -1, -1, iSellPrice, -1, -1))
						self.AfricaUnitsList.remove(transport.getID())
						screen.setSoundId(CyAudioGame().Play2DSound("AS2D_BUILD_BANK"))
					self.hideDialogBox(-1)
					...
					if (not iYield == -1) :
						...
						if (player.getGold() - iBoycottPrice) >= 0:
							CyMessageControl().sendApplyEvent(CvUtil.EventDoEuropeScreen, EventContextTypes.EVENTCONTEXT_ALL, (LIFT_BOYCOTT_IN_AFRICA, -1, -1, -1, -1, -1, -1, iYield, iBoycottPrice))
			...
			if (inputClass.getFunctionName() == "DialogTable") :
				...
				for iYield in self.YieldList :
					...
					screen.show("AfricaPrices" + str(iYield))
			#R&R, vetiarvind, Navigation sectors - START
			#elif (inputClass.getFunctionName() == "DialogMap" + "East") :
	...
	def update(self, fDelta):
		if (CyInterface().isDirty(InterfaceDirtyBits.AfricaScreen_DIRTY_BIT)):
			CyInterface().setDirty(InterfaceDirtyBits.AfricaScreen_DIRTY_BIT, False)
			self.drawContents()
	...
	def getCityInfo(self, iRow, pTransport) :
		...
		for iYield in self.YieldList :
			screen.hide("AfricaPrices" + str(iYield))
			szNumStorage = u"-"
